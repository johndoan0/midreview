...
var pipeline = function(value, ...callbacks){
    callbacks.forEach(function(callback){return value = callback(value)})
    return value
}
...
var jquerydefer = function(callback){
    var deferredobj = $.Deferred(); //creates deferred object similar to $.ajax request, setTimeout, etc.
    setTimeout(function(){deferredobj.resolve(), 1000}) //when defer obj is resolved, wait 1 sec
    return callback; // return callback after 1 sec
}
...
var Place = function(placename){
    this.fetchData = function(placename){
        var ajaxParams = {
            ...
            address: placename,
            output: 'json',
            key: 'AIzaSyDfJlGlSD_aZLkDMnteUSLP6wPUwPbcsCo'
        }

        var data = $.ajax(ajaxParams).done(function(data){return data})
    }
